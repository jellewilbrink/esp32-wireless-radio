...
String nvsgetstr ( const char* key )
{
  ...
  if ( nvserr ) {
    dbgprint ( "nvs_get_str failed %X for key %s, keylen is %d, len is %d!", nvserr, key, strlen ( key ), len );
  ...
  while ( xSemaphoreTake ( SPIsem, ctry ) != pdTRUE )        // Claim SPI bus
  {
    if ( count++ > 25 ) {
      dbgprint ( "SPI semaphore not taken within %d ticks by CPU %d,"
                 " id %s.  Claimed by %s",
                 count * ctry, xPortGetCoreID(), p, old_id );
    }
    if ( count >= 100 ) {
...
void listNetworks()
{
  ...
  for ( i = 0; i < numSsid; i++ ) {
    ...
    dbgprint ( "%2d - %-25s Signal: %3d dBm, Encryption %4s, %s", i + 1, WiFi.SSID ( i ).c_str(), WiFi.RSSI ( i ),
               getEncryptionType ( encryption ), acceptable );
...
bool updateNr ( int16_t* pnr, int16_t maxnr, int16_t nr, bool relative )
{
  ...
  dbgprint ( "updateNr %d <= %d to %d, relative is %d", *pnr, maxnr, nr, relative );
...
void nextPreset ( int16_t pnr, bool relative = false )
{
  ...
  if ( presetinfo.station_state == ST_PLAYLIST )        // In playlist mode?
  {
    if ( !updateNr ( &presetinfo.playlistnr,        // Yes, next index possible?
                     presetinfo.highest_playlistnr, pnr, relative ) ) {
  ...
  if ( presetinfo.station_state == ST_PRESET )        // In preset mode?
  {
    updateNr ( &presetinfo.preset,        // Select next preset
               presetinfo.highest_preset, pnr, relative );
...
void IRAM_ATTR timer10sec()
{
  ...
  if ( datamode & ( INIT | HEADER | DATA |        // Test op playing
                    METADATA | PLAYLISTINIT | PLAYLISTHEADER | PLAYLISTDATA ) ) {
...
void IRAM_ATTR isr_enc_turn()
{
  ...
  act_state = ( digitalRead ( ini_block.enc_clk_pin ) << 1 ) + digitalRead ( ini_block.enc_dt_pin );
...
bool connecttohost()
{
  ...
  dbgprint ( "Connect to host %s", presetinfo.host.c_str() );
  ...
  if ( presetinfo.host.endsWith ( ".m3u" ) )        // Is it an m3u playlist?
  {
    ...
    dbgprint ( "Playlist request, entry %d", presetinfo.playlistnr );
  }
  ...
  if ( mp3client->connect ( hostwoext.c_str(), port ) ) {
    ...
    if ( mp3client->connected() ) {
      sprintf ( getreq,
                ...
                "%s"        // Auth
                "Connection: close\r\n\r\n",
                extension.c_str(), hostwoext.c_str(), auth.c_str() );
...
bool connectETH()
{
  ...
  dbgprint ( "ETH pins %d, %d and %d", ini_block.eth_power_pin, ini_block.eth_mdc_pin, ini_block.eth_mdio_pin );
  res = ETH.begin ( ETH_ADDR, ini_block.eth_power_pin,        // Start Ethernet driver
                    ini_block.eth_mdc_pin, ini_block.eth_mdio_pin, ETH_TYPE, ETH_CLK_MODE );
    ...
    if ( nvssearch ( mykey ) ) {
      ...
      if ( val.length() )               // Does it exists?
      {
        if ( !touchpin[i].reserved )        // Do not use reserved pins
        {
          ...
          dbgprint ( "touch_%02d pin (GPIO%02d) is reserved for I/O!", i, pinnr );
        }
      }
    }
  }
...
String readprefs ( bool output )
{
  ...
  for ( i = 0; i < MAXKEYS; i++ )        // Loop trough all available keys
  {
    ...
    if ( output ) {
      if ( ( i > 0 ) && ( *(uint16_t*)key != last2char ) )        // New paragraph?
      {
        outstr += String ( "#\n" );        // Yes, add separator
      }
...
bool mqttreconnect()
{
  ...
  if ( res ) {
    ...
    dbgprint ( "MQTT connection failed, rc=%d", mqttclient.state() );
  }
  return res;
}
//**************************************************************************************************
  ...
  if ( nxtserial )                   // NEXTION active?
  {
    while ( nxtserial->available() )        // Yes, any input seen?
    {
      ...
      if ( c == 0xFF )                    // End of command?
      {
        ...
        if ( len ) {
          strncpy ( cmd, serialcmd.c_str(), sizeof ( cmd ) );
          dbgprint ( "NEXTION command seen %02X %s", cmd[0], cmd + 1 );
...
void scandigital()
{
  ...
  for ( i = 0; ( pinnr = progpin[i].gpio ) >= 0; i++ )        // Scan all inputs
  {
    ...
    if ( level != progpin[i].cur )                    // Change seen?
    {
      ...
      {
        dbgprint ( "GPIO_%02d is now LOW, execute %s", pinnr, progpin[i].command.c_str() );
  ...
  for ( i = 0; ( pinnr = touchpin[i].gpio ) >= 0; i++ )        // Scan all inputs
  {
    ...
    if ( level != touchpin[i].cur )        // Change seen?
    {
      ...
      if ( !level )                   // HIGH to LOW change?
      {
        dbgprint ( "TOUCH_%02d is now %d ( < %d ), execute %s", pinnr, tlevel, THRESHOLD, touchpin[i].command.c_str() );
...
void scanIR()
{
  ...
  if ( ir_value )               // Any input?
  {
    ...
    if ( nvssearch ( mykey ) ) {
      val = nvsgetstr ( mykey );        // Get the contents
      dbgprint ( "IR code %04X received. Will execute %s", ir_value, val.c_str() );
      reply = analyzeCmd ( val.c_str() );        // Analyze command and handle it
      dbgprint ( reply );                        // Result for debugging
    } else {
      dbgprint ( "IR code %04X received, but not found in preferences!  Timing %d/%d", ir_value, ir_0, ir_1 );
    }
...
String getradiostatus()
{
  return String ( "preset=" ) +                                          // Add preset setting
         String ( presetinfo.host ) + String ( "\nvolume=" ) +           // Add volume setting
         String ( ini_block.reqvol ) + String ( "\ntoneha=" ) +          // Add tone setting HA
         String ( ini_block.rtone[0] ) + String ( "\ntonehf=" ) +        // Add tone setting HF
         String ( ini_block.rtone[1] ) + String ( "\ntonela=" ) +        // Add tone setting LA
         String ( ini_block.rtone[2] ) + String ( "\ntonelf=" ) +        // Add tone setting LF
...
uint8_t FindNsID ( const char* ns )
{
  ...
  while ( offset < nvs->size ) {
    ...
    while ( i < 126 ) {
      ...
      if ( ( bm == 2 ) && ( nvsbuf.Entry[i].Ns == 0 ) && ( strcmp ( ns, nvsbuf.Entry[i].Key ) == 0 ) ) {
...
{
  dbgprint ( "Connected to host at %s on port %d", client->remoteIP().toString().c_str(), client->remotePort() );
}
//**************************************************************************************************
...
void setup()
{
  ...
  dbgprint ( "Starting ESP32-radio running on CPU %d at %d MHz.", xPortGetCoreID(), ESP.getCpuFreqMHz() );
  dbgprint ( "Version %s.  Free memory %d", VERSION,
...
// Init settings for rotary switch (if existing).
  ...
  if ( ( ini_block.enc_up_pin + ini_block.enc_dwn_pin + ini_block.enc_sw_pin ) > 2 ) {
    ...
    dbgprint (
      "ZIPPY side switch is disabled (%d/%d/%d)", ini_block.enc_up_pin, ini_block.enc_dwn_pin, ini_block.enc_sw_pin );
  }
#else
  if ( ( ini_block.enc_clk_pin + ini_block.enc_dt_pin + ini_block.enc_sw_pin ) > 2 ) {
    ...
    dbgprint (
      "Rotary encoder is disabled (%d/%d/%d)", ini_block.enc_clk_pin, ini_block.enc_dt_pin, ini_block.enc_sw_pin );
  }
#endif
  if ( NetworkFound ) {
    gettime();        // Sync time
...
void writeprefs ( AsyncWebServerRequest* request )
{
  ...
  dbgprint ( "writeprefs numargs is %d", numargs );
  for ( i = 0; i < numargs; i++ )        // Scan de parameters
  {
    ...
    dbgprint ( "Handle POST %s = %s", key.c_str(), contents.c_str() );        // Toon POST parameter
...
void handle_settings ( AsyncWebServerRequest* request )
{
  ...
  for ( i = 0; i < MAXPRESETS; i++ )        // Max number of presets
  {
    ...
    if ( statstr != "" )                     // Preset available?
    {
      ...
      val += String ( "preset_" ) + String ( i ) + String ( "=" ) + statstr + String ( "\n" );        // Add delimeter
    }
  }
#ifdef DEC_HELIX
...
void chk_enc()
{
  ...
  if ( singleclick || doubleclick ||        // Any activity?
       tripleclick || longclick || ( rotationcount != 0 ) ) {
...
void handlebyte_ch ( uint8_t b )
{
  ...
  if ( chunked && ( datamode & ( DATA |        // Test op DATA handling
...
const char* analyzeCmd ( const char* par, const char* val )
{
  ...
  } else if ( ( relative = argument.startsWith ( "down" ) ) )        // - relative setting?
  {
  ...
  } else if ( argument == "preset" )                  // (UP/DOWN)Preset station?
  {
    ...
    sprintf ( reply, "Preset is now %d",        // Reply new preset
              presetinfo.preset );
    if ( NetworkFound ) myQueueSend ( radioqueue, &startcmd );        // Signal radiofuncs()
  }
#ifdef SDCARD
...
#endif
  else if ( ( value.length() > 0 ) && ( argument == "station" ) )        // Station in the form address:port
  {
  ...
  {
    sprintf ( reply, "Free memory is %d, chunks in queue %d, bitrate %d kbps", heapspace,
              uxQueueMessagesWaiting ( dataqueue ), mbitrate );
  ...
  else if ( argument.startsWith ( "tone" ) )        // Tone command
  {
    ...
    sprintf ( reply, "Parameter for bass/treble %s set to %d", argument.c_str(), ivalue );
  ...
  } else if ( argument.startsWith ( "bat" ) )        // Battery ADC value?
  {
    ...
    sprintf ( reply, "%s called with illegal parameter: %s", NAME, argument.c_str() );
  }
...
void playtask ( void* parameter )
{
  ...
  i2s_config.dma_buf_count = 16, i2s_config.dma_buf_len = 256, i2s_config.use_apll = false;
...
#define IIC_CLK  32
  ...
  if ( ( ini_block.i2s_bck_pin + ini_block.i2s_lck_pin + ini_block.i2s_din_pin ) > 2 )        // Check for legal pins
  {
...
void sdfuncs()
{
  ...
  if ( openfile ) {
    if ( mp3filelength )        // Read until eof
    {
      ...
      {
        size_t n = read_SDCARD ( outchunk.buf, sizeof ( outchunk.buf ) );
  ...
  if ( xQueueReceive ( sdqueue, &sdcmd, 0 ) )        // New command in queue?
  {
    ...
    case QSTARTSONG:         // Start a new song?
      ...
      {
        dbgprint ( "File opened, track = %d", getCurrentSDTrackNumber() );
